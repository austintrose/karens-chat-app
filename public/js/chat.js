...
socket.on('face_change', function(data) {
	var change_host = (username == data.username ? is_host : !is_host);
	var face_img = $(change_host ? 'img#host-face-img' : 'img#client-face-img');
	var new_src = '../img/' + data.new_face + '.jpg';
...
chat_form.on('submit', function(e){
	...
	if (chat_field.val().trim().length > 0) {
		...
		scrollToBottom();
			...
			socket.emit('msg', {msg: chat_field.val(), username: username, room_id: room_id});
